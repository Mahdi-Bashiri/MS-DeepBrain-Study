"""
P4 Article - Predict Script for ventricles and WMH segmentation task

This script first prepare the input data (NIfTI) for inference and then predict the binary masks for them.
...
from scipy.ndimage import gaussian_filter, binary_dilation, label, binary_fill_holes
...
# %
def size_check(data_all, v_size, dim=(256, 256), antialias=True):
    ...
    for i in range(data_all.shape[2]):
        ...
        rescaled_data = rescale(data_, scaling_factors, anti_aliasing=antialias, mode='reflect')
    ...
    for h in range(data_img.shape[2]):
        ...
        if contours:
            ...
            if len(largest_contour) >= 5:  # 
                ...
                # ── Step 5 ──────────────────────────────────────────────────
                # Create a single object mask based on the largest contour.
                # We draw the contour filled (all interior pixels = 255), then
                # flood-fill any remaining holes so that brain tissue, CSF, and
                # skull bone are all captured as one solid foreground region.

                single_obj_mask = np.zeros_like(filled_mask)

                # Draw the largest contour as a solid white filled polygon
                cv2.drawContours(single_obj_mask, [largest_contour], 
                                 contourIdx=0, 
                                 color=255, 
                                 thickness=cv2.FILLED)

                # Fill any internal holes that cv2.drawContours may leave
                # (e.g. dark ventricles or air pockets inside the skull region)
                single_obj_mask_bool = single_obj_mask.astype(bool)
                single_obj_mask_filled = binary_fill_holes(single_obj_mask_bool)
                single_obj_mask = (single_obj_mask_filled * 255).astype(np.uint8)
                # ────────────────────────────────────────────────────────────

                
                # 6. Unpad the obtained eroded mask
                single_obj_mask_unpad = single_obj_mask[pad_width:-pad_width, pad_width:-pad_width]

                mask_img[..., h] = single_obj_mask_unpad

            else:
                print("\t\tNot enough points to acquire skull mask.")
        else:
            print("\t\tNo Contours to acquire skull mask")
...
def preprocess(
    ...
    flair_dir = os.path.join(MAIN_DIR, DATASET_NAME[:-5])
    DATASET_NAME_ = DATASET_NAME + '_prepared'
    COHORT_DIR =  "/mnt/d/TEMP_P4" # os.path.dirname(flair_dir)

    SAVE_DIR = {
    'FLAIR File Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed', 'files'),
    'Masked FLAIR File Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed', 'files_bet'),
    'FLAIR Image Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed', 'images'),
    'Masked FLAIR Image Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed', 'images_bet'),
    'Brain Mask Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed', 'Brain_Masks'),
    'Skull Mask Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed', 'Skull_Masks')
    }
    ...
    ZOOMED_SAVE_DIR = {
    'Zoomed FLAIR File Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed_zoomed', 'files'),
    'Masked Zoomed FLAIR File Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed_zoomed', 'files_bet'),
    'Zoomed FLAIR Image Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed_zoomed', 'images'),
    'Masked Zoomed FLAIR Image Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed_zoomed', 'images_bet'),
    'Zoomed Brain Mask Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed_zoomed', 'Brain_Masks'),
    'Zoom Factors Save': os.path.join(COHORT_DIR, DATASET_NAME_, 'FLAIR_Preprocessed_zoomed', 'Scaling_factors')
    }

    for name, save_directory in ZOOMED_SAVE_DIR.items():
        os.makedirs(save_directory, exist_ok=True)

    brain_mask_dir = os.path.join(MAIN_DIR, DATASET_NAME[:-5])
    ...
    for file in files:
        ...
        # if 'MS_COHORT_PREP' in DATASET_NAME:
            # for cc in range(flair_data.shape[-1]):
            #     print('\n\tShape\n')
            #     print(np.min(flair_data[..., cc]), np.max(flair_data[..., cc]))
        ...
        # nifti_data_nr = np.copy(flair_data)
        ...
        # % STEP 3
        # Simple Skull Extraction:
        skull_masks_data, skull_brain_cnt, skull_brain_ax = skull_mask_new(nifti_data_s)
        skull_masks_data = np.where(skull_masks_data < 128, 0, 1).astype(np.uint8)
        nii_skull_save_path = os.path.join(SAVE_DIR['Skull Mask Save'], file[:6] + '_skull_mask.nii.gz')
        save_nifti(skull_masks_data, flair_obj, nii_skull_save_path)
        ...
        # nii_tempo_save_path = np.copy(flair_file_path) # os.path.join(brain_mask_dir, file[:6] + '.nii.gz')
        brain_image_nii_path = fsl_bet(nii_tempo_save_path, fractional_intensity=0.45, generate_mask=True)
        os.remove(brain_image_nii_path)
        ...
        masks_data_bet = masks_data_bet > 0
        masks_data_bet = size_check(masks_data_bet, voxel_size, dim=(desired_dim, desired_dim), antialias=False)

        os.remove(nii_tempo_save_path)
        # bet_brain_mask_path = os.path.join(brain_mask_dir, file[:6] + '_brain_mask.nii.gz')
        # masks_data_bet = nib.load(bet_brain_mask_path).get_fdata()

        masks_data_2 = (masks_data * masks_data_bet).astype(np.uint8)
        save_nifti(masks_data_2, flair_obj, bet_brain_mask_path)
        ...
        flair_out_path = os.path.join(SAVE_DIR['FLAIR File Save'], file)
        save_nifti(nifti_data_z, flair_obj, flair_out_path)

        nifti_data_z_bet = np.copy(nifti_data_z)
        _brain_masks = masks_data_2 > 0
        nifti_data_z_bet[~_brain_masks] = np.min(nifti_data_z)
        flair_out_path_bet = os.path.join(SAVE_DIR['Masked FLAIR File Save'], file)
        # save_nifti(nifti_data_z_bet, flair_obj, flair_out_path_bet)
        ...
        #     # Mask by BET:
        #     masked_img_save_path = os.path.join(SAVE_DIR['Masked FLAIR Image Save'], f"{file[:6]}_{slc+1}.npy")
        #     np.save(masked_img_save_path, nifti_data_z_bet[..., slc].astype(np.float32))
        ...
        for slc in range(nifti_data_n.shape[-1]):
            ...
            masked_img_save_path = os.path.join(SAVE_DIR['Masked FLAIR Image Save'], f"{file[:6]}_{slc+1}.png")
            skimage.io.imsave(masked_img_save_path, np.uint16(65535.0 * nifti_data_n[..., slc] * masks_data_2[..., slc]))

        # Save brain info
        # np.savez(flair_out_path.replace('.nii.gz', '_binfo.npz'), brain_cnt=brain_cnt, brain_ax=brain_ax, brain_mask=masks_data)
        ...
        for i in range(nifti_data_n.shape[-1]):
            ...
            secondary_slice_images = [brain_slice]
            ...
            fit_brain_array[..., i] = fit_secondary_images[0]
        ...
        zoomed_flair_out_path = os.path.join(ZOOMED_SAVE_DIR['Zoomed FLAIR File Save'], file)
        save_nifti(fit_flair_array_z, flair_obj, zoomed_flair_out_path)

        fit_flair_array_z_bet = np.copy(fit_flair_array_z)
        _brain_masks_fit = fit_flair_array_z > 0
        fit_flair_array_z_bet[~_brain_masks_fit] = np.min(fit_flair_array_z)

        masked_zoomed_flair_out_path = os.path.join(ZOOMED_SAVE_DIR['Masked Zoomed FLAIR File Save'], file)
        save_nifti(fit_flair_array_z_bet, flair_obj, masked_zoomed_flair_out_path)
        ...
        #     masked_img_save_path = os.path.join(ZOOMED_SAVE_DIR['Masked Zoomed FLAIR Image Save'], f"{file[:6]}_{slc+1}.npy")
        #     np.save(masked_img_save_path, fit_flair_array_z_bet[..., slc].astype(np.float32))
        ...
        for slc in range(fit_flair_array.shape[-1]):
            ...
            zoomed_masked_img_save_path = os.path.join(ZOOMED_SAVE_DIR['Masked Zoomed FLAIR Image Save'], f"{file[:6]}_{slc+1}.png")
            skimage.io.imsave(zoomed_masked_img_save_path, np.uint16(65535.0 * fit_flair_array[..., slc] * fit_brain_array[..., slc]))
        ...
        zooming_factors_file_path = os.path.join(ZOOMED_SAVE_DIR['Zoom Factors Save'], f"{file[:6]}_zooming_factors.npy")
...
if __name__ == '__main__':
    ...
    MAIN_DIR = '/mnt/d/TEMP_P4'
    DATASET_NAME = 'MS_COHORT_PREP' # choose from 'HC_COHORT_PREP' or 'MS_COHORT_PREP'

    imaging_o = 'not_FS' # if DATASET_NAME=='HC_COHORT_PREP' else 'not_FS'
    ...
    MAIN_DIR = '/mnt/d/TEMP_P4'
    DATASET_NAME = 'HC_COHORT_PREP' # choose from 'HC_COHORT_PREP' or 'MS_COHORT_PREP'
