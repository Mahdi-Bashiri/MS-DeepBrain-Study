"""
P4 Article - Core Processing Script for ventricles and WMH masks detailing

This script focuses on producing three sub types of the WMH mask as Periventricular, Paraventricular, and Juxtacortical masks.
...
def obj_analyzer(
    ...
    Parameters:
        ...
        c_mask (np.ndarray): Binary CSF or GM slice mask.
...
# %%
def main_process(gm_pred_path, vent_pred_path, wmh_pred_path, brain_mask_path, flair_path, result_dir, p_id, save_rgb_images=False):
    ...
    try:
        ...
        # Load Raw Main FLAIR Data
        flair_data, flair_obj = load_nifti(flair_path)

        desired_dim = 256
        voxel_size_main_flair = np.array(flair_obj.header['pixdim'][1:4])
        voxel_size_main_flair[-1] = 1

        flair_data = noise_red(flair_data)
        flair_data = size_check(flair_data, voxel_size_main_flair, dim=(desired_dim, desired_dim))
        flair_data = (flair_data - np.min(flair_data)) / (np.max(flair_data) - np.min(flair_data))
        flair_data = (65535.0 * flair_data).astype(np.uint16)

        # Morphologically post-processing the four masks.

        # for specialized Gray Matter masks:
        gm_data, gm_obj = load_nifti(gm_pred_path)
        gm = np.where(gm_data > 0, 1, 0).astype(np.uint8)

        # for ventricle masks:
        vent_data, vent_obj = load_nifti(vent_pred_path)
        vent = np.where(vent_data > 0, 1, 0).astype(np.uint8)

        # for WMH masks:
        wmh_data, wmh_obj = load_nifti(wmh_pred_path)
        wmh = np.where(wmh_data > 0, 1, 0).astype(np.uint8)

        # for brain masks:
        brain_data, brain_obj = load_nifti(brain_mask_path)
        brain = np.where(brain_data > 0, 1, 0).astype(np.uint8)
        ...
        gm_masks = np.copy(gm)
        vent_masks = np.copy(vent)
        wmh_masks = np.copy(wmh)
        brain_masks = np.copy(brain)

        # dilate the rbain masks for increasing the inclusion probability of CSF
        for i in range(brain_masks.shape[-1]):
            brain_masks[..., i] = binary_dilation(brain_masks[..., i] > 0, disk(2))
        brain_masks *= 1

        # prepare CSF mask
        # generate CSF-candidate masks:
        csf_masks = generate_adaptive_dark_masks(flair_data) * brain_masks
        csf_masks = np.where(csf_masks > 0, 1, 0).astype(np.uint8)
        print(np.min(csf_masks), np.max(csf_masks), np.sum(csf_masks))
        ...
        # %
        # constant values:
        vent_rule = 10                # enter max distance from ventricles allowed
        gm_rule = (4, 4)              # enter (Distance from GM mask, max Diameter of allowed objects)
        
        # Define color codes (RGB values)
        color_codes = {
            'red': (255, 0, 0),         # peri_found
            'orange': (255, 115, 0),    # para_found
            'yellow': (255, 255, 0),    # juxt_found
            'blue': (0, 0, 255),        # vent_mask
            'gray': (64, 64, 64),       # brain_mask
            'light_blu': (40, 50, 255), # csf_mask
            'light_gry': (150, 150, 150), # gm_mask
        }

        # global interested_slc_high, interested_slc_low
        # interested_slc_low = 9          # lower slice to be processed
        # interested_slc_high = 15        # upper slice to be processed
        ...
        peri_found_masks = np.zeros_like(flair_data, dtype=np.uint8)
        para_found_masks = np.copy(peri_found_masks)
        juxt_found_masks = np.copy(peri_found_masks)

        for slice_idx in range(flair_data.shape[-1]):   # Assuming shape (H, W, num_slices)

            print(f"\n\n\t\t\t\t S L I C E : {slice_idx + 1}  of  Patient : {p_id}\n")

            gm_mask = gm_masks[..., slice_idx]
            vent_mask = vent_masks[..., slice_idx]
            wmh_mask = wmh_masks[..., slice_idx]
            brain_mask = brain_masks[..., slice_idx]
            csf_mask = csf_masks[..., slice_idx]

            (peri_found, para_found, juxt_found,
            areas_code, contours_wmh,
            number_wmh, areas_wmh
            ) = obj_analyzer(
                wmh_mask, vent_mask, c_mask=gm_mask,
                voxel_size=voxel_size, vent_rule=vent_rule, gm_rule=gm_rule
                )

            peri_found_masks[..., slice_idx] = peri_found
            para_found_masks[..., slice_idx] = para_found
            juxt_found_masks[..., slice_idx] = juxt_found

            all_wmh_area_code.append(areas_code)
            all_wmh_num.append(number_wmh)
            all_wmh_area.append(areas_wmh)

            if save_rgb_images:
                # Initialize RGB mask
                rgb_mask = np.zeros((wmh_masks.shape[0], wmh_masks.shape[1], 3), dtype=np.uint8)

                # Apply colors to the RGB mask
                # Order matters - later assignments will overwrite earlier ones where masks overlap

                # Gray for brain
                rgb_mask[brain_mask == 1] = color_codes['gray']

                # Light Blue for CSF
                rgb_mask[csf_mask == 1] = color_codes['light_blu']

                # Light Green for GM
                rgb_mask[gm_mask == 1] = color_codes['light_gry']
                ...
                rgb_masks_save_dir = os.path.join(result_dir, 'rgb_masks')
                os.makedirs(rgb_masks_save_dir, exist_ok=True)
                rgb_masks_save_path = os.path.join(rgb_masks_save_dir, f"{p_id}_{slice_idx + 1}_rgb_mask.png")
                skimage.io.imsave(rgb_masks_save_path, rgb_mask)

        # %
        # Save the results for further analysis:
        peri_masks_save_dir = os.path.join(result_dir, 'peri_masks')
        os.makedirs(peri_masks_save_dir, exist_ok=True)
        peri_masks_save_path = os.path.join(peri_masks_save_dir, f"{p_id}_peri_mask.nii.gz")
        save_nifti(peri_found_masks, wmh_obj, peri_masks_save_path)
        
        para_masks_save_dir = os.path.join(result_dir, 'para_masks')
        os.makedirs(para_masks_save_dir, exist_ok=True)
        para_masks_save_path = os.path.join(para_masks_save_dir, f"{p_id}_para_mask.nii.gz")
        save_nifti(para_found_masks, wmh_obj, para_masks_save_path)
        
        juxt_masks_save_dir = os.path.join(result_dir, 'juxt_masks')
        os.makedirs(juxt_masks_save_dir, exist_ok=True)
        juxt_masks_save_path = os.path.join(juxt_masks_save_dir, f"{p_id}_juxt_mask.nii.gz")
        save_nifti(juxt_found_masks, wmh_obj, juxt_masks_save_path)

        # Save the analysis results in a single file
        # Create a dictionary to store all results
        results_data = {
            'areas_code': all_wmh_area_code,
            'number_wmh': all_wmh_num,
            'areas_wmh': all_wmh_area,
            'color_codes': color_codes,  # Include color mapping for reference
            'mask_info': {
                'peri_count': np.sum(peri_found_masks),
                'para_count': np.sum(para_found_masks),
                'juxt_count': np.sum(juxt_found_masks),
                'vent_count': np.sum(vent_masks)
            }
        }

        # Create results filename (same base name as image file)
        pickle_save_dir = os.path.join(result_dir, 'pickles')
        os.makedirs(pickle_save_dir, exist_ok=True)
        results_filename = os.path.join(pickle_save_dir, f'{p_id}_results.pkl')

        # Save as pickle file (handles different shapes/types well)
        with open(results_filename, 'wb') as f:
            pickle.dump(results_data, f)

        if save_rgb_images: print(f"RGB mask saved to: {rgb_masks_save_dir}")
...
if __name__ == "__main__":
    ...
    for cohort_group in cohort_groups:
        ...
        # Directory of ventricle_WMH prediction files in actual dimensions
        gm_predictions_dir = f"/mnt/d/TEMP_P4/{cohort_group}_COHORT_PREP_prepared/FLAIR_Preprocessed/GM_Masks"
        vent_predictions_dir = f"/mnt/d/TEMP_P4/{cohort_group}_COHORT_PREP_prepared/FLAIR_Preprocessed/Vent_Masks"
        wmh_predictions_dir = f"/mnt/d/TEMP_P4/{cohort_group}_COHORT_PREP_prepared/FLAIR_Preprocessed/WMH_Masks"
        brain_masks_dir = f"/mnt/d/TEMP_P4/{cohort_group}_COHORT_PREP_prepared/FLAIR_Preprocessed/Brain_Masks"

        # Directory of preprocessed FLAIR data in actual dimensions
        flair_dir = f"/mnt/d/TEMP_P4/{cohort_group}_COHORT"

        # Directory of resulting masks
        result_dir = f"/mnt/d/TEMP_P4/{cohort_group}_COHORT_PREP_prepared/FLAIR_Preprocessed/Sub_Masks"
        os.makedirs(result_dir, exist_ok=True)

        gm_pred_files = [f for f in os.listdir(gm_predictions_dir) if f.endswith('.gz')]
        vent_pred_files = [f for f in os.listdir(vent_predictions_dir) if f.endswith('.gz')]
        wmh_pred_files = [f for f in os.listdir(wmh_predictions_dir) if f.endswith('.gz')]
        brain_mask_files = [f for f in os.listdir(brain_masks_dir) if f.endswith('.gz')]
        flair_files = [f for f in os.listdir(flair_dir) if f.endswith('.gz')]

        for g_pred_file, v_pred_file, w_pred_file, b_mask_file in zip(gm_pred_files, vent_pred_files, wmh_pred_files, brain_mask_files):
            # Load prediction file
            gm_pred_path = os.path.join(gm_predictions_dir, g_pred_file)
            vent_pred_path = os.path.join(vent_predictions_dir, v_pred_file)
            wmh_pred_path = os.path.join(wmh_predictions_dir, w_pred_file)
            brain_mask_path = os.path.join(brain_masks_dir, b_mask_file)

            for flair_file in flair_files:
                if flair_file[:6] == v_pred_file[:6]:
                    flair_path = os.path.join(flair_dir, flair_file)
                    break
            
            p_id = v_pred_file.split('_')[0]

            # if p_id != '110214': continue

            # Call the core processing
            main_process(gm_pred_path, vent_pred_path, wmh_pred_path, brain_mask_path, flair_path, result_dir, p_id, save_rgb_images=True)
