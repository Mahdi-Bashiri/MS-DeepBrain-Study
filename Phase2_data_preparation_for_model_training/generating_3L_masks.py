...
def mapping_3l(min, max):

    """
    (0.0*(max-min) + min):background ,
    (0.5*(max-min) + min):n_wmh ,
    (1.0*(max-min) + min):ab_wmh
    """
    return [(0.0*(max-min) + min), (0.5*(max-min) + min), (1.0*(max-min) + min)]
...
flair_dir = r"D:\PhD Students\MBashiri\ours_articles\Paper#2\Data\Final_Original_FLAIRs"
# Directory of Manually Segmented Abnormal WMH Data
man_seg_wmh_dir = r"D:\PhD Students\MBashiri\ours_articles\Paper#2\Data\Final_WMH_Segmentations"
# Directory of Manually Segmented Ventricles Data
man_seg_vent_dir = r"D:\PhD Students\MBashiri\ours_articles\Paper#2\Data\Final_Vent_Segmentations"
# Directory of Generated 3-level Masks Data
save_path = r"D:\PhD Students\MBashiri\ours_articles\Paper#2\Data\man_3L_masks_wmhs"
os.makedirs(save_path, exist_ok=True)
png_save_path_0 = r"D:\PhD Students\MBashiri\ours_articles\Paper#2\Data\man_3L_masks_wmhs_png_all"
png_save_path_1 = r"D:\PhD Students\MBashiri\ours_articles\Paper#2\Data\man_3L_masks_wmhs_png_nonZero"
png_save_path_2 = r"D:\PhD Students\MBashiri\ours_articles\Paper#2\Data\man_3L_masks_wmhs_png_Zero"
os.makedirs(png_save_path_0, exist_ok=True)
os.makedirs(png_save_path_1, exist_ok=True)
os.makedirs(png_save_path_2, exist_ok=True)

# ## Make a 3L Mask and Save it
...
for file in files:
    ...
    # ## Excluding WMH Masks from Ventricles
    vent_data = vent_data & ~ab_wmh_data

    # ## Creating a 3-level Mask

    mask_points = np.uint16(mapping_3l(min=0, max=65535))

    mask_3l = vent_data * mask_points[1] + ab_wmh_data * mask_points[2]

    # ## Show and Save Results
    # nii_display(vent_data*65535, n_wmh_data*65535, ab_wmh_data*65535, mask_4l)

    # Save the created 3L mask. You might want to copy the header/affine from the original data.
    save_nifti(mask_3l, vent_obj, os.path.join(save_path, file))
    # # Save the updated masks.
    # save_nifti(vent_data, vent_obj, os.path.join(man_seg_vent_dir, file))

# ## Save PNG Images from 3L Masks
...
for file in files:
    ...
    data_3L, obj_3L = load_nifti(os.path.join(save_path, file))

    # nii_display(data_flair, data_3L, data_flair, data_3L)
    # Go in loop
    for i in range(data_flair.shape[-1]):
        ...
        fit_flair, fit_3l, b_mask = fitter(im_fl, im_3l)

        # nii_display_1(im_fl, im_3l, fit_flair, fit_3l)
        ...
        merged_image = np.concatenate((np.rot90(fit_flair, 1), np.rot90(fit_3l, 1)), axis=1).astype(np.uint16)

        # if np.sum(fit_3l) == 0:  # to separate the empty 4L masks from the rest
        ...
        dst_path = os.path.join(png_save_path_0, file.replace('.nii.gz', f'_{i + 1}.png'))

        skimage.io.imsave(dst_path, merged_image)

# ## Separate the paired images to test and train sets based on 80% & 20% random selection.

# Define paths
train_folder = os.path.join(os.path.dirname(save_path) , 'train_3L_wmh_vent')
test_folder = os.path.join(os.path.dirname(save_path) , 'test_3L_wmh_vent')
...
ims = os.listdir(png_save_path_0)
...
test_ids = ['104252', '105074', '105465', '105755', '105911',
            '110497', '112378', '112657', '113046', '114525']

# Move images to respective folders
for file in ims:
    img_id = file[:6]
    src_path = os.path.join(png_save_path_0, file)
