...
class MSStatisticalAnalysis:
    ...
    def correlation_analysis(self):
        ...
        numeric_vars = [
            ...
            'deep_wmh_ratio', 'juxta_wmh_ratio', self.config.COLUMNS['age'], self.config.COLUMNS['sex']
        ]
        numeric_vars_names = [
            'Ventricle Ratio', 'WMH Ratio',
            'Peri WMH Ratio', 'Deep WMH Ratio', 'Juxta WMH Ratio',
        ...
        with pd.ExcelWriter(excel_filename, engine='openpyxl') as writer:
            ...
            fig.suptitle('Standardized Correlation Analysis with Method Selection', fontsize=18, fontweight='bold')
            ...
            for idx, group in enumerate(['HC', 'MS']):
                ...
                if group == "HC":
                    ...
                    quantity = 381
                ax_corr.set_title(f'{panel_letter_corr}.    {group} Group - Correlation Coefficients (n={quantity})',
                                fontsize=18, fontweight='bold')
                # ax_corr.set_title(f'{group} Group - Correlation Coefficients (n={len(numeric_data_clean)})',
                #                   fontsize=14, fontweight='bold')
                ax_corr.set_xticklabels(numeric_vars_names, rotation=45, ha='right', fontsize=16)
                ax_corr.set_yticklabels(numeric_vars_names, rotation=0, fontsize=16)
                ...
                #                     fontsize=14, fontweight='bold')
                ax_method.set_xticklabels(numeric_vars_names, rotation=45, ha='right', fontsize=16)
                ax_method.set_yticklabels(numeric_vars_names, rotation=0, fontsize=16)
                        ...
                        if i < j:  # Only upper triangle to avoid duplicates
                            ...
                            if detailed_result:
                                ...
                                summary_data.append({
                                    ...
                                    # Bonferroni: 21 unique pairs in 7×7 matrix → α = 0.05/21 ≈ 0.0024
                                    'Bonferroni_alpha': round(0.05 / 21, 4),
                                    'Bonferroni_n_comparisons': 21,
                                    'Significant (Bonferroni α=0.0024)': 'Yes' if p_val < (0.05 / 21) else 'No',                                    'Group': group,
        ...
        for group in ['HC', 'MS']:
            ...
            for var_column, var_display_name in key_correlations:
                if var_column in group_data_clean.columns:
                    ...
                    if result:
                        # Bonferroni threshold: 5 age-related correlations tested per group
                        # (VentricleRatio, WMHRatio, peri_wmh_ratio, deep_wmh_ratio, juxta_wmh_ratio)
                        # → α_bonferroni = 0.05 / 5 = 0.01  (Reviewer 2, Minor Comment 6)
                        _n_age_comparisons = len(key_correlations)
                        _alpha_bonferroni = 0.05 / _n_age_comparisons
                        _p = result['p_value']
                        ...
                        print(f"  P-value: p = {_p}")
                        print(f"  Effect Size (strength): {result['strength']}")
                        print(f"  Sample Size: n = {result['n_samples']}")
                        print(f"  Significant (uncorrected α = 0.05): {'Yes' if _p < 0.05 else 'No'}")
                        print(f"  Bonferroni correction: α = 0.05 / {_n_age_comparisons} comparisons "
                              f"= {_alpha_bonferroni:.4f}")
                        print(f"  Significant (Bonferroni α = {_alpha_bonferroni:.4f}): "
                              f"{'Yes' if _p < _alpha_bonferroni else 'No'}")
        ...
        self.results['correlation_analysis_standardized'] = {
            ...
            'methodology': {
                ...
                'significance_levels': {
                    'uncorrected': 0.05,
                    # Age correlations: 5 variables tested per group → 0.05/5 = 0.01
                    'bonferroni_age_correlations': round(0.05 / 5, 4),
                    'bonferroni_age_n_comparisons': 5,
                    # Full 7×7 correlation matrix upper triangle: C(7,2)=21 pairs → 0.05/21 ≈ 0.0024
                    'bonferroni_full_matrix': round(0.05 / 21, 4),
                    'bonferroni_full_matrix_n_comparisons': 21,
                }
            }
        }
    ...
    def create_summary_table(self):
        ...
        summary_data = {
            ...
            'Healthy Controls': [
                ...
                f"{hc_data['deep_wmh_ratio'].mean():.3f} ± {hc_data['deep_wmh_ratio'].std():.3f}",
            ...
            'MS Patients': [
                ...
                f"{ms_data['deep_wmh_ratio'].mean():.3f} ± {ms_data['deep_wmh_ratio'].std():.3f}",
...
class MSAdvancedVisualizations:
    ...
    def create_publication_figure_1(self):
        ...
        ax4.set_xlabel('Age (years)', fontsize=16)
        ax4.set_ylabel('Number of Patients', fontsize=16)
        ax4.set_title('D. Age Distribution by Group and Gender', fontsize=18, fontweight='bold')
        ax4.legend()
    ...
    def create_publication_figure_2(self):
        ...
        def plot_burden_unified(ax, data_filter, data_column, y_label, title_prefix, title_suffix="",
            ...
            for group in ['HC', 'MS']:
                ...
                # ── Scatter plot (all data points regardless of age limit) ──────────────────
                scatter = ax.scatter(ages_clean, ratios_clean, alpha=0.6, s=50, color=color,
                                     label=f'{group} (n={len(ages_clean)})')
                ...
                # Reviewer 1 Comment 11: sparse 60+ data may distort trends.
                # REGRESSION_AGE_LIMIT clips the rendered trend line (not the data).
                # Reviewer 2 Minor Comment 7: polynomial degree and R² reported in legend.
                _poly_degree = 2
                _age_limit = getattr(self.config, 'REGRESSION_AGE_LIMIT', None)

                if len(ages_clean) >= 3:
                    try:
                        # Fit on ALL data (full age range) for an unbiased coefficient estimate
                        age_mean = ages_clean.mean()
                        age_std = ages_clean.std()
                        ages_normalized = (ages_clean - age_mean) / age_std if age_std > 0 else ages_clean

                        z = np.polyfit(ages_normalized, ratios_clean, _poly_degree)
                        poly_fn = np.poly1d(z)

                        # Compute R² on the fitted data
                        fitted_vals = poly_fn(ages_normalized)
                        ss_res = np.sum((ratios_clean - fitted_vals) ** 2)
                        ss_tot = np.sum((ratios_clean - ratios_clean.mean()) ** 2)
                        r_squared = 1.0 - ss_res / ss_tot if ss_tot > 0 else np.nan

                        # Determine upper age for the rendered trend line
                        plot_age_max = min(ages_clean.max(),
                                          float(_age_limit)) if _age_limit is not None else ages_clean.max()
                        age_range = np.linspace(ages_clean.min(), plot_age_max, 100)
                        age_range_normalized = (age_range - age_mean) / age_std if age_std > 0 else age_range
                        trend_values = poly_fn(age_range_normalized)

                        # Legend label includes degree and R² (Reviewer 2 Minor Comment 7)
                        age_limit_str = (f", ≤{int(_age_limit)}y" if _age_limit is not None
                                         and _age_limit < ages_clean.max() else "")
                        trend_label = (f"{group} trend (poly-{_poly_degree}, "
                                       f"R²={r_squared:.3f}{age_limit_str})")
                        ax.plot(age_range, trend_values, '--', color=color, linewidth=2, alpha=0.8,
                                label=trend_label)

                        print(f"    {group}: poly-{_poly_degree} fit, "
                              f"R²={r_squared:.3f}, "
                              f"trend rendered {ages_clean.min():.0f}–{plot_age_max:.0f} y "
                              f"(data range {ages_clean.min():.0f}–{ages_clean.max():.0f} y)")

                        # Store regression data
                        panel_data['regression_results'][group] = {
                            'polynomial_coefficients': z.tolist(),
                            'polynomial_degree': _poly_degree,
                            ...
                            'r_squared': r_squared,
                            'age_limit_applied': _age_limit,
                            'data_age_min': float(ages_clean.min()),
                            'data_age_max': float(ages_clean.max()),
                            'trend_age_max': float(plot_age_max),
                    ...
                    panel_data['regression_results'][group] = {
                        'regression_successful': False,
                        'error_message': (f'Insufficient data points '
                                          f'(n={len(ages_clean)}, minimum required=3)')
                    }

            ax.set_xlabel('Age (years)', fontsize=16)
            ax.set_ylabel(y_label, fontsize=16)
            if panel_id:
                ax.set_title(f'{panel_id}.    {title_prefix}{title_suffix}', fontsize=18, fontweight='bold')
            else:
            ...
            for group in ['HC', 'MS']:
                if group in panel_info['regression_results']:
                    ...
                    if reg_data['regression_successful']:
                        _r2 = reg_data['r_squared']
                        _age_lim = reg_data.get('age_limit_applied', 'N/A')
                        regression_rows.append({
                            'Panel': panel_id,
                            'Group': group,
                            'Regression_Successful': 'Yes',
                            'Polynomial_Degree': reg_data.get('polynomial_degree', 2),
                            'R_Squared': f"{_r2:.4f}" if isinstance(_r2, float) and not np.isnan(_r2) else 'N/A',
                            'Data_Age_Min': f"{reg_data.get('data_age_min', 'N/A')}",
                            'Data_Age_Max': f"{reg_data.get('data_age_max', 'N/A')}",
                            'Trend_Age_Max': f"{reg_data.get('trend_age_max', 'N/A')}",
                            'Age_Limit_Config': str(_age_lim),
                            'Coefficient_0_(const)': f"{reg_data['polynomial_coefficients'][2]:.6f}" if len(
                                reg_data['polynomial_coefficients']) > 2 else 'N/A',
                            'Coefficient_1_(linear)': f"{reg_data['polynomial_coefficients'][1]:.6f}" if len(
                                reg_data['polynomial_coefficients']) > 1 else 'N/A',
                            'Coefficient_2_(quadratic)': f"{reg_data['polynomial_coefficients'][0]:.6f}" if len(
    ...
    def create_publication_table_1(self):
        ...
        table_data = {
            ...
            'Healthy Controls': [
                ...
                calculate_stats(hc_data, 'TotalBrainArea', 'mean_sd'),
                ...
                calculate_stats(hc_data, 'deep_wmh_ratio', 'median_iqr'),
            ...
            'MS Patients': [
                ...
                calculate_stats(ms_data, 'TotalBrainArea', 'mean_sd'),
                ...
                calculate_stats(ms_data, 'deep_wmh_ratio', 'median_iqr'),
        ...
        p_values.append(f"{age_test.pvalue}")
        ...
        p_values.append(f"{p_gender}")

        # Other comparisons
        for var in ['TotalBrainArea', 'VentricleRatio']:
            test = stats.ttest_ind(hc_data[var], ms_data[var])
            p_values.append(f"{test.pvalue}")

        for var in ['WMHRatio', 'peri_wmh_ratio', 'deep_wmh_ratio', 'juxta_wmh_ratio']:
...
if __name__ == "__main__":
    ...
    if analyzer is not None:
        ...
        if results and 'demographics' in results:
            ...
            if 'age_test' in demo:
                print(f"Age difference (HC vs MS): p = {demo['age_test'].pvalue}")
            if 'gender_test' in demo:
                print(f"Gender distribution difference: p = {demo['gender_test'][1]}")
